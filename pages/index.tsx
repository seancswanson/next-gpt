...
import ResizableSection from "../components/ResizableSection";
  ...
  const gudea = Gudea({ weight: "400", subsets: ["latin-ext"] });
...
const renderLargeScreen = () => (
  ...
  const renderSmallScreen = () => (
    <>
      ...
      <div className={`${showSidebarSmall ? 'h-2/3' : 'h-full'} flex  w-full flex-col p-4`}>
        <button
          className="mb-2 bg-blue-500 p-2 text-white"
  ...
  return (
    <div
      className={`${gudea.className} h-screen flex-col overflow-hidden lg:flex lg:flex-row text-xl`}
    >
